...
SELECT
    C.ID, C.TITLE, C.REGION, C.CREATED_AT, C.FINISHED_AT, C.CITY_ID, C.MEMBER_ID, M.NICKNAME CREATOR, CI.NAME CITY, COUNT(CM.MEMBER_ID) COUNT
FROM CHAT_ROOM C
	LEFT JOIN CHAT_MEMBER CM ON C.ID = CM.CHAT_ROOM_ID
	LEFT JOIN MEMBER M ON C.MEMBER_ID = M.ID
	LEFT JOIN CITY CI ON C.CITY_ID = CI.ID
	GROUP BY
	C.ID, C.TITLE, C.REGION, C.CREATED_AT, C.FINISHED_AT, C.CITY_ID, C.MEMBER_ID, M.NICKNAME, CI.NAME;