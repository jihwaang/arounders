...
SELECT B.ID, B.TITLE, B.CONTENT, B.REGION, B.CREATED_AT, B.UPDATED_AT, B.IS_HIDDEN,
       ...
       (SELECT COUNT(ID) FROM COMMENT WHERE BOARD_ID = B.ID) COMMENT_COUNT,
       (SELECT COUNT(MEMBER_ID) FROM LIKES WHERE BOARD_ID = B.ID) LIKE_COUNT,
       (SELECT COUNT(MEMBER_ID) FROM INTERESTS WHERE BOARD_ID = B.ID) INTEREST_COUNT,
       M.NICKNAME WRITER, CT.TITLE CATEGORY_TITLE, CI.NAME CITY
FROM BOARD B
